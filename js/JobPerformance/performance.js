...
window.onload = function () {
    ...
    for (var i = 0; i < 3; i++) {
        ...
        html += '<td>2021.01.11</td>'
        html += '<td>NOR-CAL PRODUCT VIETNA</td>'
        html += '<td>J21L7576</td>'
        html += '<td>ID 용접</td>'
        html += '<td>191</td>'
        html += '<td>445</td>'
        html += '<td>0.004</td>'
        html += '<td>AM350</td>'
        html += '<td>04450191-MKB04021-OD</td>'
        html += '<td>OD 용</td>'
        html += '<td>리샤오리</td>'
        html += '<td>ID Lasesr 1호</td>'
        html += '<td>3150</td>'
        html += '<td>250</td>'
        html += '<td>2020.12.08</td>'
        html += '<td>2020.01.08</td>'
        html += '<td>13:25</td>'
        html += '<td>13:55</td>'
        html += '<td>185</td>'
        html += '<td>2PLY(ID)</td>'
...
function search_() {
    if (serach_date_condition() == true && search_etc_condition() == false) {
        ...
        var job_no = $('#job_order_number').val();
        var product_code = $('#product_number').val();
...
function search_etc_condition() {
    if ($('#job_order_number').val() != '' || $('#product_number').val() != '') {
        return true;
    }
    else {
        return false;
...
function make_excel_file() {
    ...
    try {
        ...
        var table = document.getElementById('job_manage_table');
        ...
        for (var i = 0; i < tr.length; i++) {
            ...
            var params = {
                ...
                '작업일자	': td[1].textContent,
                '거래처': td[2].textContent,
                'JOB NO.': td[3].textContent,
                '공정명': td[4].textContent,
                'ID': td[5].textContent,
                'OD': td[6].textContent,
                '사용소재두께': td[7].textContent,
                'Bellows Material': td[8].textContent,
                '품번': td[9].textContent,
                '품명': td[10].textContent,
                '작업자': td[11].textContent,
                '설비': td[12].textContent,
                '계획수량': td[13].textContent,
                '작업수량': td[14].textContent,
