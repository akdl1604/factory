...
function Order_add() {
    ...
    var html = '';
    cnt++;
    html += '<div class="order_box box sel">';
    html += '    <input type="hidden" id= "order_box_idx"  value="' + cnt + '">' //공정 식별 키
...
function Next() {
    if ($(".order_box").length >= 2) {
        ...
        for (var i = 0; i<process_arr.length; i++) {
            ...
            params2.PROCESS_NAME.push(process_arr[i][1].PROCESS_NAME);
            params2.PROCESS_CODE.push(process_arr[i][1].PROCESS_CODE);
            
            var params4 = {
                "ORIGIN_PROCESS_ID" : process_arr[i][1].ORIGIN_PROCESS_ID2
            }
    
            //원본 공정 매칭 서식 가져오기
            fun_ajax("POST", "http://220.89.167.212:8085/testing05/SelectMatchingProcess", params4, false, function (data3) {
                for(var i = 0; i<data3.length; i++){
                    params2.KEY.push(data3[i].JOBORDER_FORM_DETAIL_ID);
                }
            });
        }
        ...
        opener.Self_data_insert(params2);
        //window.parent.postMessage({ type : 'movetab', childData : '3.registration_preset.html', arr : params2, processarr: process_arr, last_posi: last_position }, '*');
        //ClosePopup();
    }
    else {
...
window.addEventListener('message', function(e) {
	if(e.data.type == "movepre"){
        ...
        for(var i = 0; i<process_arr.length; i++){
            ...
            html += '    <input type="hidden" id= "order_box_idx"  value="' + i + '">' //공정 식별 키
